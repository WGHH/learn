...
function setNavLevel2And3Pos(){
	...
	for(let i = 0; i < level1LiArr.length; i++){
        ...
        // setNavLevelPos($level2, $($level2.find("ul.level3")));
	}
}

/** 调节子级分类位置，根据父级在页面上的位置，往左或往右边放（不相对父级水平居中）
 * 父级添加，解决获取不到子级宽度的问题：onmouseover="setNavLevelPos($(this), $(this).find('ul.level3'));"
 * @param {object} parentLevel jquery对象
 * @param {object} childLevelArr jquery对象数组
 * @param {Boolean} isSide 位置在父级的侧边，不能再正下方，比如二级分类是位于一级分类的正下方
 */
function setNavLevelPos($parentLevel, $childLevelArr, isSide=true){
    ...
    for(let i = 0; i < $childLevelArr.length; i++){
        ...
        if(isSide) console.log(parentLevel_Left, parentLeft_to_end_distance, $parentLevel.outerWidth(true), $childLevel.outerWidth(true))
        // if($("#nav > ul > li:nth-child(18) > ul > li:nth-child(2) > ul")[0] == $childLevel[0]){
        // }
    }
}
