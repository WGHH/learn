...
function setNavLevel2Pos(){
	...
	for(let i = 0; i < level1LiArr.length; i++){
		...
		if(level1Left_to_end_distance <= $level2.outerWidth(true)){
			...
			$level2.css("right", "50%").css("left", "auto");
			$($level2).css("transform", translateX);
			...
			$level2.css("left", "50%").css("right", "auto");
			$level2.css("transform", translateX);
		}
	}
}
...
```
// 头部固定，内容不够，页面闪动的
if($("body")[0].scrollHeight-($('.pc-header .nav-container')[0].clientHeight)*3<=(window.innerHeight)) {
